...
class EMAPullbackRS(BaseStrategy):
    ...
    def _evaluate(self, ticker: str, df: pd.DataFrame, spy_df: pd.DataFrame) -> ScanHit | None:
        """Check if a single ticker meets all criteria."""
        p = self.parameters
        close = df["Close"]
        volume = df["Volume"]
        ...
        if latest < dma_mid_val or latest < dma_long_val:
            return None

        # 2) DMAs rising (current > 5 bars ago)
        if float(dma_mid.iloc[-1]) <= float(dma_mid.iloc[-6]):
            return None
        if float(dma_long.iloc[-1]) <= float(dma_long.iloc[-6]):
            return None

        # 3) Pulling back toward 8 or 21 EMA (within proximity_pct)
        ...
        # Avoid division by zero — if SPY flat, any positive stock return is RS
        if spy_ret == 0:
        ...
        # 5) Volume contraction on pullback
        avg_vol_20 = float(volume.iloc[-20:].mean())
        recent_vol = float(volume.iloc[-3:].mean())  # last 3 bars
        ...
        # Recent swing low (lowest low in last 10 bars)
        ...
        signal = {
            ...
            "suggested_stop": round(swing_low - 0.10, 2),  # just below swing low
        }
